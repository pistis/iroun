  ...
  const { directoryAction } = await inquirer.prompt([
    {
      ...
      type: 'list',
      message: `Clone target directory ${chalk.cyan(targetDir)} already exists. Pick an action:`,
      choices: [
  ...
  const { input } = await inquirer.prompt([
    {
      name: 'input',
      type: 'list',
      message: `Enter the path of your project or the URL of your Github. Pick an action:`,
      choices: [
...
async function prepare(projectName) {
  ...
  let targetDir
  if (gitUrl) {
    targetDir = path.resolve(cwd, '.gitrepo')
